        ...
        <div className="relative w-full max-w-4xl bg-white sm:bg-white/95 sm:backdrop-blur-xl rounded-none sm:rounded-[2.5rem] shadow-none sm:shadow-[0_40px_100px_rgba(0,0,0,0.2)] sm:border border-white/20 overflow-hidden sm:overflow-hidden animate-zoom-in flex flex-col h-full sm:h-auto sm:max-h-[85vh] pointer-events-auto acc-modal-content font-manrope">
          ...
          <div className="p-6 sm:p-8 border-t border-slate-100/50 bg-slate-50/30 flex flex-row sm:flex-row items-center justify-between gap-3 sm:gap-0 shrink-0 acc-modal-mobile-taskbar">
            <div className="text-[9px] sm:text-[10px] font-bold text-acc-text-light uppercase tracking-widest hidden sm:flex items-center gap-3">
              <span aria-hidden="true" className="material-symbols-outlined text-sm">verified_user</span> Được xác thực bởi Hizo Group
            </div>
...
const InvoiceList = () => {
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [searchTerm, filterStatus, timeframe, filterWeek, filterYear, filterDate, selectedDay, filterYearsCount]);
  ...
  const filteredInvoices = invoices.filter(inv => {
    ...
    return (filterStatus === 'all' || inv.orderStatus === filterStatus) && matchesSearch && matchesTime;
  });
      ...
      <div className="flex flex-col gap-2 sm:gap-3 px-1 shrink-0">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
          <p className="text-sm sm:text-base text-acc-text-muted font-medium flex flex-wrap items-center gap-x-2 gap-y-1">
            ...
            <span className="inline-flex items-center align-middle px-2.5 py-0.5 rounded-lg bg-blue-50 text-acc-primary font-bold whitespace-nowrap animate-fade-in" key={filteredInvoices.length}>
              {filterStatus === 'all' ? `Tổng: ${filteredInvoices.length} HĐ` : `${filterStatus}: ${filteredInvoices.length} HĐ`}
            </span>
          </p>

          {/* Bộ lọc Thời gian + Nút tạo */}
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full sm:w-auto sm:justify-end relative z-50">
            ...
            <button
              ...
              className="acc-btn-primary w-full sm:w-fit justify-center px-4 py-3 shadow-md font-black uppercase rounded-xl flex items-center gap-2 transition-all active:scale-95"
              style={{ fontSize: 'clamp(9px, 0.75vw, 11px)' }}
              aria-label="Tạo hóa đơn mới"
            >
              <span aria-hidden="true" className="material-symbols-outlined font-bold" style={{ fontSize: '18px' }}>post_add</span>
              <span className="font-black uppercase tracking-widest">Tạo hóa đơn mới</span>
            </button>
          </div>
        </div>
      </div>

      <div className="bg-white rounded-xl sm:rounded-2xl p-4 sm:p-6 border border-slate-200 shadow-sm flex flex-col sm:flex-row gap-3 items-center justify-between hover:border-acc-primary/30 hover:shadow-lg transition-all duration-300 shrink-0">
        <div className="relative flex-1 w-full sm:w-[400px] sm:flex-none group">
          <input 
            ...
            aria-label="Tìm kiếm mã hóa đơn hoặc khách hàng"
        ...
        <div className="relative w-full sm:w-72 sm:flex-none font-manrope">
          <button
            type="button"
          ...
          >
            ...
            <span className={`material-symbols-outlined text-slate-400 transition-transform duration-300 ${isOpenStatusDropdown ? 'rotate-180 text-acc-primary' : ''}`} style={{ fontSize: '18px' }} aria-hidden="true">
              keyboard_arrow_down
            </span>
          </button>
