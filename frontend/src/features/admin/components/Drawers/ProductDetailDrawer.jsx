...
const ProductDetailDrawer = ({ open, onClose, product, onEdit, onDelete }) => {
  ...
  useEffect(() => {
    if (open && (product?.productID || product?.id)) {
      fetchProductHistory();
    }
  }, [open, product?.productID, product?.id]);

  const fetchProductHistory = async () => {
    setLoading(true);
    const prodID = product.productID || product.id;
    try {
      // 1. Tải toàn bộ đơn hàng (sử dụng dbData directly hoặc qua API Admin nếu có)
      const allOrders = dbData.orders || [];
      
      // 2. Lấy toàn bộ hóa đơn từ dbData.invoices
      const allInvoices = dbData.invoices || [];
      
      // 3. Lọc order items thuộc về sản phẩm này
      ...
      const productOrderItems = allOrderItems.filter(oi => Number(oi.productID) === Number(prodID));
      
      // 4. Lọc ra orders và invoices liên quan
      ...
      // 5. Tạo lịch sử hoạt động giả lập cho sản phẩm ở Admin (nhập kho, xuất kho bán hàng)
      const mockActs = [];
      
      // Hoạt động nhập kho ban đầu
      mockActs.push({
        title: 'Khởi tạo sản phẩm',
        time: formatDate(product.createdAt || '2026-01-01'),
        user: 'Hệ thống',
        color: 'bg-indigo-500',
        desc: `Tạo sản phẩm với số lượng tồn ban đầu`,
      });

      // Hoạt động từ đơn hàng
      productOrders.forEach(o => {
        const item = productOrderItems.find(oi => Number(oi.orderID) === Number(o.orderID));
        mockActs.push({
          title: 'Xuất kho bán hàng',
          time: formatDate(o.orderDate || '2026-01-01'),
          user: 'Nhân viên bán hàng',
          color: 'bg-emerald-500',
          desc: `Xuất bán ${item ? item.quantity : 1} ${product.unit || 'cái'} (Đơn hàng #${o.orderID})`,
        });
      });

      setActivities(mockActs);
    } catch (error) {
      console.error("Lỗi khi tải lịch sử sản phẩm bên Admin:", error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const stats = useMemo(() => {
    ...
    const totalRevenue = history.orderItems.reduce((sum, oi) => sum + ((oi.quantity || 0) * (product.price || product.salePrice || 0)), 0);
  ...
  const getStatusText = (status) => {
    const s = String(status || '').trim();
    if (s === 'Còn hàng' || s === 'ACTIVE') return 'Còn hàng';
    if (s === 'Sắp hết' || s === 'LOW_STOCK') return 'Sắp hết';
    if (s === 'Hết hàng' || s === 'OUT_OF_STOCK') return 'Hết hàng';
    return s || 'Ngừng kinh doanh';
  };

  const displayPrice = product.price || product.salePrice || 0;
  const displayStock = product.stock !== undefined ? product.stock : (product.stockQuantity !== undefined ? product.stockQuantity : 0);
          ...
          <Box className="flex items-start gap-5 mb-6">
            ...
            <Box className="flex-1">
              ...
              <Typography className="text-xs font-bold text-slate-400 uppercase tracking-widest font-inter mb-2">
                SKU: {product.productCode || product.sku || `PRD-${String(product.productID || product.id).padStart(3, '0')}`}
              </Typography>
              <Box className="flex gap-2 items-center flex-wrap">
                <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl border-2 font-inter ${getStatusStyle(product.status)}`}>
                  <div className={`w-2 h-2 rounded-full ${getStatusText(product.status) === 'Còn hàng' ? 'bg-emerald-500' : getStatusText(product.status) === 'Sắp hết' ? 'bg-orange-500' : 'bg-rose-500'}`}></div>
          ...
          <Box className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
            <div className="bg-white p-4 rounded-2xl border border-slate-300 shadow-sm flex items-center justify-between">
              <div>
                ...
                <p className="text-lg font-black text-[#00288E] font-inter"><VNDDisplay value={displayPrice} className={CURRENCY_CLASS_PRIMARY} /></p>
              </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                  ...
                  <div className="bg-gradient-to-br from-emerald-50 to-teal-50/30 p-4 rounded-xl border border-emerald-100">
                    ...
                    <p className="text-lg font-black text-emerald-700 font-inter">
                      <VNDDisplay value={stats.totalRevenue} className={CURRENCY_CLASS_SECONDARY} />
                    </p>
                  </div>
                </div>
                <div className="space-y-4">
