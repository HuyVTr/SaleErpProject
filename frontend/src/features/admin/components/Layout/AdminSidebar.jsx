...
const AdminSidebar = ({ isOpen, onClose }) => {
  ...
  const menuItems = [
    { name: 'Bảng điều khiển', path: '/admin/dashboard', icon: 'dashboard', end: true },
    { name: 'Nhân sự', path: '/admin/staffs', icon: 'person' },
    { name: 'Quản lý Sản phẩm', path: '/admin/products', icon: 'inventory_2' },
    { name: 'Bảng giá', path: '/admin/price-lists', icon: 'sell' },
    ...
    <>
      ...
      `}>
          ...
          <div className="flex items-center justify-between relative z-10">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-blue-200/60" style={{ fontSize: '0.8125rem', lineHeight: '1.1rem', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.05rem' }}>Phân hệ Quản trị</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <nav className="flex-1 space-y-2 overflow-y-auto scrollbar-none" style={{ padding: '0 1.5rem' }}>
          <p className="text-blue-300/40" style={{ padding: '0 1rem', marginBottom: '1rem', fontSize: '0.8125rem', lineHeight: '1.1rem', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.05rem' }}>Quản trị & Danh mục</p>
          {menuItems.map((item) => (
            <NavLink
              key={item.path}
              to={item.path}
          ...
          <div className="bg-white/5 backdrop-blur-md rounded-3xl border border-white/10 relative overflow-hidden group" style={{ padding: '1.5rem' }}>
            <div className="flex items-center gap-4" style={{ marginBottom: '1.5rem' }}>
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-blue-200/50 truncate" style={{ fontSize: '0.8125rem', lineHeight: '1.1rem', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.05rem' }}>{user.roleName || 'Quản trị viên'}</p>
              </div>
            </div>
