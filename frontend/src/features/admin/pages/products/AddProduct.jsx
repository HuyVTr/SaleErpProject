...
const AddProduct = () => {
  ...
  const initialSnapshot = useRef(JSON.stringify({
    productName: '', sku: '', category: defaultCategories[0], price: '', stock: '0', unit: units[0], status: 'Còn hàng', description: ''
  }));
  ...
  const handleSubmit = (e) => {
    ...
    showModal(
      ...
      `Bạn chuẩn bị thêm sản phẩm mới "${productName.trim()}" vào hệ thống. Xác nhận lưu thông tin?`,
      'confirm',
      () => {
        setLoading(true);
        const payload = {
        ...
        adminService.createProduct(payload)
          ...
          .catch(err => {
            ...
            showModal('Thêm sản phẩm thất bại', 'Có lỗi xảy ra trong quá trình kết nối máy chủ. Vui lòng thử lại.', 'error');
          })
          .finally(() => {
            setLoading(false);
          });
      ...
      <div className="relative flex flex-col gap-2 sm:gap-3 px-2 md:px-0">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
          <p className="text-xs sm:text-sm text-slate-500 font-medium leading-relaxed">
            Thiết lập sản phẩm mới trong kho hàng
          </p>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col xl:flex-row gap-8 mx-2 md:mx-0">
          ...
          <div className="xl:flex-[2] flex flex-col gap-8">
            ...
            <div className="bg-white rounded-[2.5rem] p-10 shadow-sm border border-slate-300 hover:shadow-xl transition-[border-color,box-shadow] duration-500">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-2">
                  <label htmlFor="stock" className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] ml-1">Số lượng tồn kho ban đầu</label>
                  <input 
                    id="stock"
                    type="number" 
                    min="0"
            ...
            <div className="bg-white rounded-[2.5rem] p-10 shadow-sm border border-slate-300 hover:shadow-xl transition-[border-color,box-shadow] duration-500">
              ...
              <div className="space-y-2">
                <textarea 
                  ...
                  className="w-full bg-slate-50 border-2 border-transparent rounded-[2rem] p-5 text-sm font-bold outline-none focus:border-blue-500 focus:bg-white transition-[border-color,background-color] text-slate-700 font-inter focus-visible:ring-2 focus-visible:ring-blue-600"
                />
              </div>
            </div>
          </div>
