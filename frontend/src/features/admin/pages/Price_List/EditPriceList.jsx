  ...
  useEffect(() => {
    ...
    Promise.all([adminService.getPriceLists(), adminService.getProducts(), adminService.getCustomers()]).then(([list, products, customers]) => {
      ...
      const found = arr.find(item => {
        ...
        return itemId === cleanTargetId || String(item.id || item.listID) === String(id);
      });
      if (!found) {
      ...
      const rawDate = found.effectiveDate || found.startDate || '';
      setEffectiveDate(rawDate ? String(rawDate).split('T')[0] : '');
      ...
      adminService.getPriceListItems(found.id).then(savedItems => {
        ...
        const loadedItems = arr2.map((it, idx) => {
          ...
          return {
            id: itemId,
            productID: pId,
            productName: product?.productName || it.productName || it.products?.[0]?.productName || `Sản phẩm #${pId}`,
            unit: product?.unit || it.unit || it.products?.[0]?.unit || 'cái',
